...
export type Run = { id: string; agentId: string; status: RunStatus; startedAt: string; endedAt: string | null; error: string | null };
export interface NewsroomAPI { listAgents(): Promise<Agent[]>; listMessages(agentId: string): Promise<Message[]>; startRun(agentId: string): Promise<Run>; postMessage(agentId: string, text: string): Promise<never>; updateDraft(id: string, update: Partial<Pick<Draft, 'facebookText'|'xText'|'reviewStatus'>>): Promise<never>; }
