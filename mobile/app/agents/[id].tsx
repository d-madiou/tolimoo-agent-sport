...
export default function AgentScreen() {
  ...
  const researchActive = Boolean(agent?.isRunning || starting || run?.status === 'queued' || run?.status === 'running');
  return <ScrollView contentContainerStyle={styles.container} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => void load(true)} tintColor="#276EF1" />}>
    ...
    {!loading && agent ? <>
      ...
      {run && !agent.isRunning ? <Text style={styles.runNotice}>Recherche mise en file d’attente à {formatDate(run.startedAt)}.</Text> : null}
      <Pressable accessibilityRole="button" disabled={researchActive || !agent.enabled} onPress={() => void startResearch()} style={({ pressed }) => [styles.startButton, (researchActive || !agent.enabled) && styles.startDisabled, pressed && styles.pressed]}>{starting ? <ActivityIndicator color="white" /> : <Text style={styles.startText}>{researchActive ? 'Recherche en cours' : 'Lancer une recherche'}</Text>}</Pressable>
      ...
      {messages.map((message) => <View key={message.id} style={styles.message}>
        <View style={styles.messageMeta}><Text style={styles.role}>{roleLabel[message.role]}</Text><Text style={styles.date}>{formatDate(message.createdAt)}</Text></View>
        <Text style={styles.messageText}>{message.text}</Text>
        {message.draft ? <View style={styles.draft}>
          <Text style={styles.draftEyebrow}>{reviewLabel[message.draft.reviewStatus]} · {claimLabel[message.draft.claimStatus]}</Text><Text style={styles.headline}>{message.draft.headline}</Text>
          <Text style={styles.platform}>FACEBOOK</Text><Text style={styles.copy}>{message.draft.facebookText}</Text>
          <Text style={styles.platform}>X</Text><Text style={styles.copy}>{message.draft.xText}</Text>
          <Text style={styles.sourceHeading}>Sources ({message.draft.sources.length})</Text>
          {message.draft.sources.map((source) => <Pressable key={source.id} onPress={() => safeOpen(source.url)} style={styles.source} accessibilityRole="link"><Text style={styles.sourceTitle}>{source.title}</Text><Text style={styles.sourceUrl} numberOfLines={1}>{source.url}</Text></Pressable>)}
        </View> : null}
      </View>)}
    </> : null}
  </ScrollView>;
...
const styles = StyleSheet.create({
  container: { flexGrow: 1, padding: 20, paddingBottom: 42, backgroundColor: '#F7F8FC', gap: 14 }, loader: { marginTop: 38 }, hero: { paddingTop: 8, gap: 8 }, eyebrow: { color: '#276EF1', fontSize: 12, fontWeight: '800', letterSpacing: 1.3 }, title: { color: '#101828', fontSize: 28, lineHeight: 35, fontWeight: '800' }, subtitle: { color: '#667085', fontSize: 15, lineHeight: 22 }, statusLine: { flexDirection: 'row', alignItems: 'center', gap: 7, marginTop: 5 }, statusDot: { width: 8, height: 8, borderRadius: 8 }, statusLive: { backgroundColor: '#14B87A' }, statusIdle: { backgroundColor: '#98A2B3' }, statusText: { color: '#475467', fontSize: 13, fontWeight: '600' }, demo: { color: '#745800', backgroundColor: '#FFF3CD', borderRadius: 10, padding: 12 }, runNotice: { color: '#174EA6', backgroundColor: '#EEF4FF', borderRadius: 10, padding: 12, lineHeight: 20 }, startButton: { minHeight: 50, alignItems: 'center', justifyContent: 'center', backgroundColor: '#276EF1', borderRadius: 12, paddingHorizontal: 18 }, startDisabled: { backgroundColor: '#9BBEF8' }, startText: { color: 'white', fontSize: 16, fontWeight: '800' }, pressed: { opacity: 0.8 }, disabledText: { color: '#667085', fontSize: 13 }, sectionHeading: { flexDirection: 'row', alignItems: 'baseline', justifyContent: 'space-between', marginTop: 12 }, sectionTitle: { color: '#101828', fontSize: 18, fontWeight: '700' }, count: { color: '#667085', fontSize: 13 }, empty: { color: '#667085', backgroundColor: 'white', borderRadius: 14, borderWidth: 1, borderColor: '#E6E8EF', padding: 16, lineHeight: 22 }, errorPanel: { gap: 10, padding: 16, borderRadius: 14, backgroundColor: '#FFF1F1' }, errorText: { color: '#A61B1B', lineHeight: 21 }, retryText: { color: '#A61B1B', fontWeight: '800' }, message: { backgroundColor: 'white', borderRadius: 16, padding: 16, gap: 10, borderWidth: 1, borderColor: '#E6E8EF' }, messageMeta: { flexDirection: 'row', justifyContent: 'space-between', gap: 10 }, role: { color: '#276EF1', fontSize: 12, fontWeight: '800', letterSpacing: 0.7, textTransform: 'uppercase' }, date: { color: '#98A2B3', fontSize: 12 }, messageText: { color: '#344054', lineHeight: 21 }, draft: { gap: 9, backgroundColor: '#F8FAFF', borderLeftColor: '#276EF1', borderLeftWidth: 3, borderRadius: 6, padding: 13 }, draftEyebrow: { color: '#475467', fontSize: 12, fontWeight: '700' }, headline: { color: '#101828', fontSize: 19, lineHeight: 25, fontWeight: '800' }, platform: { color: '#667085', fontSize: 11, fontWeight: '800', letterSpacing: 1.1, marginTop: 3 }, copy: { color: '#344054', lineHeight: 21 }, sourceHeading: { color: '#344054', fontSize: 13, fontWeight: '800', marginTop: 5 }, source: { borderTopWidth: 1, borderColor: '#E6E8EF', paddingTop: 9, gap: 3 }, sourceTitle: { color: '#174EA6', fontSize: 14, fontWeight: '700' }, sourceUrl: { color: '#667085', fontSize: 12 },
});
